    ...
    case "$INIT_SYSTEM" in
        runit)
            if [ -d "/etc/runit/sv/$service_name" ]; then
                # Create symlink in the active runlevel, not directly in /run/runit/service
                ln -sf "/etc/runit/sv/$service_name" "/etc/runit/runsvdir/default/$service_name"
            fi
            ;;
        s6)
...
case "$INIT_SYSTEM" in
    runit)
        # Remove symlink from the runlevel directory, not from /run/runit/service
        rm -f /etc/runit/runsvdir/*/connmand 2>/dev/null || true
...
EOF
  ...
  # Create a down file to prevent auto-start
