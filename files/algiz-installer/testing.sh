...
add_service() {
    ...
    case "$INIT_SYSTEM" in
        runit)
            if [ -d "/etc/runit/sv/$service_name" ] && [ -x "/etc/runit/sv/$service_name/run" ]; then
                ln -sf "/etc/runit/sv/$service_name" "/run/runit/service/$service_name"
                sv start "$service_name" 2>/dev/null || true
            else
                echo "Warning: Service $service_name not found or missing run script"
            fi
            ;;
        s6)
...
case "$INIT_SYSTEM" in
    runit)
        rm -f /run/runit/service/connmand
...
EOF
  ...
  # Enable the service
  ln -sf /etc/runit/sv/rc.local /run/runit/service/rc.local
fi
