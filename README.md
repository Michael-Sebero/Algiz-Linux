...
Disk and SSD performance is tuned through scheduler and queue optimizations. SSDs use the `none` scheduler for zero-overhead I/O, while HDDs use `bfq` for better fairness under mixed workloads. NVMe drives bypass the scheduler entirely (none) for maximum throughput. Read-ahead is increased to 512 KB for SSDs and 2048 KB for HDDs for improved sequential performance, while I/O queue depth is raised to 1024 for SATA SSDs and 2048 for NVMe, enabling higher parallelism. HDDs are configured with a queue depth of 128. I/O request merging is enabled to combine adjacent requests for efficiency.
